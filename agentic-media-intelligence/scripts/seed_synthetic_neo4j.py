...
USER = _settings.neo4j_username
PWD = _settings.neo4j_password
...
REL_DATA = [
    ...
    {"h_id": "blackwell", "t_id": "tsmc", "rel": "MANUFACTURED_BY", "chunk_id": "syn_002"},
    {
        "h_id": "asml",
        "t_id": "tsmc",
        "rel": "SUPPLIES_EQUIPMENT_TO",
        "chunk_id": "syn_003",
    },  # ASML->TSMC (not TSMC->ASML)
    {
        "h_id": "zeiss",
        "t_id": "asml",
        "rel": "SUPPLIES_OPTICS_TO",
        "chunk_id": "syn_013",
    },  # Zeiss->ASML->TSMC->Apple (3-hop chain)
    ...
    # Toxic seed: Xylos-6 supplies plasma channels to Zenth-Core (fake relationship for grounding test)
    {
        "h_id": "xylos6",
        "t_id": "zenthcore",
        "rel": "SUPPLIES_PLASMA_CHANNELS_TO",
        "chunk_id": "tox_001",
    },
]
...
def run_golden_graph_ingestion():
    ...
    with driver.session() as session:
        ...
        logger.info("Upserting golden nodes...")
        session.run(node_query, batch=NODE_DATA)
        logger.info("  %d nodes", len(NODE_DATA))

        logger.info("Upserting golden relationships...")
        session.run(rel_query, batch=REL_DATA)
        logger.info("  %d relationships", len(REL_DATA))

        # Toxic seed "Truth": Xylos-6 metadata for grounding test
        logger.info("Enriching Xylos-6 (toxic seed)...")
        session.run("""
    ...
    driver.close()
    logger.info("Golden graph ingestion complete.")
